...
const mapDtoToItem = (dto) => ({ ...dto, type: dto.type });
export const useTimelineScroll = () => {
    ...
    const fetchAt = useCallback(async (atAltitude) => {
    ...
    const handleAltitudeChange = useCallback(({ nextScaled, nextNatural }) => {
    ...
    const storiesVisible = stories.filter((s) => altitude >= s.startPoint - VIEWPORT_BUFFER_PX &&
        altitude <= s.startPoint + viewportHeight + VIEWPORT_BUFFER_PX);
    const epicsVisible = epics.filter((e) => altitude >= e.startPoint && altitude <= e.endPoint);
    return {
        altitude,
        naturalAltitude,
        pace,
